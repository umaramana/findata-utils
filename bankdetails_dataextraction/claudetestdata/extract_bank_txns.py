...
def main():
    ...
    for i, img_path in enumerate(image_files):
        ...
        if txns:
            ...
            parsed_sub = sum(float(t['subtracted'].replace(',', '')) for t in txns if t['subtracted'])
            parsed_add = sum(float(t['added'].replace(',', '')) for t in txns if t['added'])

            if expected_sub is not None:
                sub_gap = expected_sub - parsed_sub
                add_gap = expected_add - parsed_add
                if abs(sub_gap) > 0.02 or abs(add_gap) > 0.02:
                    ...
                    # Add sentinel placeholder row for the unaccounted gap
