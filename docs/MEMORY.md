...
- **Location**: `review/` (committed c904994, 2026-09-21; client run folders, `review_runs_names.json`, the venv and the user's two spec `.txt` files stay untracked) + `redactor/` (OCR + 9-digit rule committed ddda057). Open items: `review/PARKING_LOT.md`. Status log: `review/SPEC-revision-extraction.md`.
...
- **Redactor** (`redactor/redact.py`): SSN/EIN plus any standalone 9-digit number (`ID9`, catches undashed EINs; also 9-digit account numbers/CUSIPs) plus entered names and addresses; XMP and non-widget annotations deleted; output filenames scrubbed; name variants auto-matched. Names are typed only in the user's own terminal (`review/redact.py --prompt`), never stored, never seen by Claude.
- **OCR (`--ocr`, `redactor/redact_ocr.py`, Tesseract)**: image-only pages are OCR'd, matched pixels blacked out, output re-OCR'd; up to 2 extra passes when only OCR re-reads find something (`passes=N`). Unreadable pages (<5 words, confidence <60, sideways pixels) stay REVIEW. Weaker guarantee than a text layer. ~1-2 min per scanned page.
- **Other tools**: `review/img_to_pdf.py` (JPEG/PNG photos -> one-page PDFs, then `redact.py --prompt --ocr`); `review/check4_run.py` (masked Path A driver: own gate, `--dry-run`, index-only output; run by the USER, the API key lives only in their shell). W-2 box 2 compares to line 25a.
- **Data-safety rules**: Claude never reads client names or page text. Printed filename lines count as sensitive. FAIL/REVIEW outputs are never opened and never left in a folder given to `check4_run.py`. Before committing under `review/`: explicit filenames only, then grep the staged diff for names/emails/SSN- and key-shaped strings. The auto-mode classifier blocks ad-hoc page-text dumps; masked probes (booleans, geometry, token shapes) are allowed.
- **Run**: `cd review && .venv/bin/python -m unittest test_drake test_redact_wrapper test_img_to_pdf` (51 tests). Redactor tests are a script: `cd redactor && ../review/.venv/bin/python test_redact.py` (95 checks), not unittest.
- **Decisions (user, 2026-09-21)**: Path A's no-third-party-names rule is lifted for THIS client only (SSN/EIN/typed names still removed); model `claude-sonnet-5`; gate = the user's terminal result plus the driver's own generic re-check.
- **State**: `client_bh1` redacted 11/11 OK (return is file #9). The 3 phone-photo sources went through `img_to_pdf` + `--ocr`: 1099-INT and one W-2 OK, one W-2 FAILED (OCR variance; moved unopened to `review_runs_redacted/client_bh1_img_FAILED_do_not_send/`). NEXT: user re-redacts that W-2 alone, then `check4_run.py --dry-run`, then the real run.
- **Still open**: Path A never run on the API (risks: `max_tokens` 2048, 100 DPI page images, no §5 eligibility gate); Path B fails on real multi-page docs (context overflow); `.gitignore` lacks `review_runs_redacted/` and `review_runs_names.json`; redact.py prints nothing until all files finish.
