...
**Git Bash mangles `/secrets/...` paths in `--set-secrets`/`--set-env-vars` (2026-08-04).** If you run the step-4 `gcloud run deploy` command from Git Bash (MSYS), its automatic POSIX-to-Windows path conversion silently rewrites `/secrets/oauth_token.json` into something like `C:/Program Files/Git/secrets/oauth_token.json` before `gcloud` ever sees it — no error at deploy time, but the container then fails every Sheets read with `RuntimeError: OAuth token not found at C:/Program Files/Git/secrets/oauth_token.json` (visible only in Cloud Run logs; the API response is the same generic `Could not read client data from Sheets.` as an expired OAuth token, so don't assume token expiry without checking logs first). `MSYS_NO_PATHCONV=1` does NOT fix this reliably (broke gcloud's own shim path in testing); doubling the leading slash (`//secrets/...`) survives Git Bash but `gcloud` itself rejects that literal path. **Fix: run this specific `gcloud run deploy` command from PowerShell, not Git Bash.** Verify after any redeploy with `gcloud run services describe report-service --region us-central1 --format="value(spec.template.spec.containers[0].env)"` — the printed value must be exactly `/secrets/oauth_token.json`.
