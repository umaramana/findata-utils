...
def _render_qc_results(qc_result):
    if 'error' in qc_result:
        ...
        if qc_result.get('review_rows'):
            st.warning(f"**{len(qc_result['review_rows'])} row(s) need manual review:**")
    ...
    with st.expander("Sheet Processing Diagnostics", expanded=True):
        ...
        if broker_key.startswith('csv_'):
            ...
            for sn in diag_excel.sheet_names:
                ...
                sheet_count = len(processed_df[processed_df['Source Sheet'] == sn]) if 'Source Sheet' in processed_df.columns else 'N/A'
...
def _render_summary_metrics(summary):
    ...
    if has_fed_tax:
        ...
        col5.metric("Fed Tax Withheld", f"${summary['fed_tax_total']:,.2f}",
                    help="Enter this total manually in Drake.")
    else:
    ...
    if 'Fed Tax Withheld' in display_df.columns:
        def _highlight_fed_tax(row):
            ...
            try:
                ...
                pass
            return [''] * len(row)
        st.dataframe(display_df.style.apply(_highlight_fed_tax, axis=1), use_container_width=True, height=400)
    else:
...
def _render_processing_error(e):
    ...
    st.markdown("Check the file format matches the expected broker format.")
