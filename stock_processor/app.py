...
def main():
    ...
    **Prerequisites:**
    ...
    if uploaded_file is not None:
        ...
        # Auto-run column alignment check (cached per file + broker)
        # csv_ brokers are native CSV exports — no PDF24 artifacts, skip QC
        ...
        # Process button — use corrected Excel if available
...
def process_file(uploaded_file, broker, broker_key):
    ...
    try:
        with st.spinner(f"Processing {broker} file..."):
            # Route to appropriate broker processor
            if broker == "Fidelity":
                processed_df = fidelity.process(uploaded_file)
            elif broker == "Charles Schwab":
                processed_df = schwab.process(uploaded_file)
            elif broker == "Robinhood":
                processed_df = robinhood.process(uploaded_file)
                ...
                processed_df = jpmorgan.process(uploaded_file)
            else:  # Betterment
                processed_df = betterment.process(uploaded_file)

            # Normalize Fed Tax column name across brokers
            # (some brokers may use 'Federal Tax withheld' or similar variants)
            ...
            # Show raw processed data (collapsed)
            ...
            # Per-sheet diagnostic breakdown
