...
| `eligibility.py` | §5 eligibility gate, document-level only (see SPEC.md §5.2) - excludes consolidated 1099s, Schedule K-1s, 1099-OID by content before extraction, so they're never OCR'd or sent. Wired into `check4_run.py`, ahead of the redaction gate. Return-level eligibility (Sch E/F, 1099-S, etc.) is not covered. |
| `page_filter.py` | Page-level filter within an otherwise in-scope document - drops IRS copy duplicates (Copy A/B/C/D/1/2, same box values on W-2/1099-INT/1099-DIV) before the model call, so a repeated copy can't inflate a repeatable field. Wired into `extract.py`'s `extract_api`/`extract_local`. Dropped pages are logged (`ExtractionResult.dropped_pages`), never silently discarded. |
